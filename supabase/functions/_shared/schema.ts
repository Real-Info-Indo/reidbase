...
- For ranking queries ("which area has the highest yield", "best yield locations", etc.): JOIN reid_rentals with reid_properties on location to compute gross_yield_pct = (median_adr * 365 * avg_occupancy / 100) / median_price_usd * 100. Use LEFT JOIN on LOWER(TRIM(location)). Filter properties to available listings only. Example:
  SELECT r.location, r.type, ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY r.rate_usd)) AS median_adr, ROUND(AVG(r.occupancy), 1) AS avg_occupancy, ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY p.price_usd)) AS median_price_usd, ROUND((PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY r.rate_usd) * 365 * AVG(r.occupancy) / 100.0) / NULLIF(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY p.price_usd), 0) * 100, 1) AS gross_yield_pct, COUNT(*) AS n, SUM(r.count) AS total_properties FROM reid_rentals r LEFT JOIN reid_properties p ON LOWER(TRIM(r.location)) = LOWER(TRIM(p.location)) AND p.availability = 'Available' AND p.price_usd IS NOT NULL AND TO_DATE(p.scrape_date, 'Mon/YY') >= (SELECT MAX(TO_DATE(scrape_date, 'Mon/YY')) - INTERVAL '5 months' FROM reid_properties) WHERE TO_DATE(r.date, 'Mon/YY') >= (SELECT MAX(TO_DATE(date, 'Mon/YY')) - INTERVAL '11 months' FROM reid_rentals) GROUP BY r.location, r.type HAVING COUNT(*) >= 3 ORDER BY gross_yield_pct DESC NULLS LAST LIMIT 15
