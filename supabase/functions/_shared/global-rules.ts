...
- Product and tier naming: the product is REID Base. Paid tiers are Member, Pro, and Enterprise. Use "REID Base Member", "REID Base Pro", "REID Base Enterprise". Unsubscribed users are classified internally as "Freemium". Do not use informal labels such as "basic plan" or "paid tier".
- Never use the word "Freemium" in any user-facing output. It is an internal classification only. When describing access limits, reference the user's current access level by name (Member, Pro, Enterprise) or use "your current plan". Never name a lower tier to explain what a user cannot access.
