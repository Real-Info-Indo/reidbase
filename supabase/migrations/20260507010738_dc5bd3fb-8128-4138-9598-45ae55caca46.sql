-- Extend admin_analytics_summary with full-history retention metrics.
-- retention_snapshot and weekly_retention_cohorts are computed from the
-- ENTIRE analytics_events table, ignoring the (p_from, p_to) window.
...
BEGIN
  ...
  WITH chat_part AS (
    ...
    SELECT mode FROM chat_part UNION SELECT mode FROM event_part
  )
  ...
  FROM (
    SELECT lower((regexp_match(metadata->>'referrer', '^[a-z]+://([^/]+)'))[1]) AS host,
      ...
      FROM public.analytics_events
     WHERE event_type = 'page_view' AND created_at >= p_from AND created_at <= p_to
       AND metadata ? 'referrer' AND length(COALESCE(metadata->>'referrer','')) > 0
     GROUP BY 1
  ...
  -- Compute over the entire analytics_events table, not the dashboard window.
  -- Sessions are derived from per-user event timelines: a new session begins
  -- when there is a >= 24h gap between consecutive events for that user.
  WITH user_events AS (
  ...
  per_user_returning AS (
    -- A user is "returning" if they have a session that starts >= 24h after first_seen.
  ...
  -- Weekly retention cohorts (full history). Cohort = ISO week (WITA) of
  -- first-ever event. Retained = at least one event >= 24h after first_seen.
  WITH user_events AS (
    SELECT wix_user_id, created_at
      FROM public.analytics_events
     WHERE wix_user_id IS NOT NULL
