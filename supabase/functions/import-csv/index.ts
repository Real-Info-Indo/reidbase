...
serve(async (req) => {
  ...
  // Phase 1B: import endpoints are admin-only. Caller must present a valid
  // Wix bearer token AND be in public.admin_users.
  let identity;
  try {
  ...
  try {
    ...
    if (truncate === true) {
      // Wipe the table before importing the first chunk. Use a filter that
      // matches every row so PostgREST accepts the delete.
