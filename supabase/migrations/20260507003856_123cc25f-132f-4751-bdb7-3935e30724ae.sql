...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'appraisal_submissions',(SELECT count(*) FROM ev WHERE event_type = 'feature' AND event_name = 'appraisal_submitted')
  ) INTO v_summary;

  -- ── Page views by WITA day ──────────────────────────────────────────────
  ...
  SELECT jsonb_build_object(
    ...
    'report_view',         (SELECT count(*) FROM public.analytics_events
                             WHERE event_type = 'feature' AND event_name = 'funnel_report_view'
                               AND created_at >= p_from AND created_at <= p_to),
    'appraisal_submitted', (SELECT count(*) FROM public.analytics_events
                             WHERE event_type = 'feature' AND event_name = 'appraisal_submitted'
                               AND created_at >= p_from AND created_at <= p_to)
  ) INTO v_funnel;
  ...
  -- ── Top referrers (external hostnames only) ─────────────────────────────
  ...
  -- ── Top campaigns (utm_source / utm_medium / utm_campaign) ──────────────
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  -- ── New appraisal count (status = 'new', not range-bound) ───────────────
